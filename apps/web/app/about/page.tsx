...
export default function About() {
  return (
    <main id="top" className="relative min-h-screen bg-[#05070b] text-neutral-300">
      {/* Slow-drifting colour behind the whole page. Three hues, because a
          page built from one blue reads as a template. */}
      ...
      {/*
        One failing section must never blank the page. This is what turns a
        crash into a missing block — and the deployed site proved the
        difference matters.
      */}
      <div className="relative z-10">
        <ErrorBoundary area="Navigation" fallback={null}>
          <SiteNav />
        </ErrorBoundary>
        <ErrorBoundary area="Hero" fallback={null}>
          <LiveHero />
        </ErrorBoundary>
        <ErrorBoundary area="Problem" fallback={null}>
          <Problem />
        </ErrorBoundary>
        <ErrorBoundary area="HowItWorks" fallback={null}>
          <HowItWorks />
        </ErrorBoundary>
        <ErrorBoundary area="Results" fallback={null}>
          <Results />
        </ErrorBoundary>
        <ErrorBoundary area="Maps" fallback={null}>
          <Maps />
        </ErrorBoundary>
        <ErrorBoundary area="Ai" fallback={null}>
          <Ai />
        </ErrorBoundary>
        <ErrorBoundary area="EdgeEngine" fallback={null}>
          <EdgeEngine />
        </ErrorBoundary>
        <ErrorBoundary area="Honest" fallback={null}>
          <Honest />
        </ErrorBoundary>
        <ErrorBoundary area="Download" fallback={null}>
          <Get />
        </ErrorBoundary>
        <Footer />
      </div>
  ...
  return (
    <Section id="problem" eyebrow="The problem" title="Satellite signals cannot pass through concrete.">
      ...
      <p className="mt-6 max-w-2xl text-[13px] leading-relaxed text-neutral-500">
        High-end cars carry factory-fitted inertial systems wired to the wheels.
        Trucks, older cars and millions of two-wheelers have nothing but the
        phone on the dashboard — so it has to work from that alone.
      </p>
    </Section>
  );
}
...
function Results() {
  return (
    <Section id="results" eyebrow="Measured" title="10.0% mean drift — and the tail we do not hide.">
      ...
      <div className="mt-9">
        <p className="mb-5 max-w-2xl text-[13.5px] leading-relaxed text-neutral-400">
          Each row changes exactly one component, so every gain is attributable
          to something specific. Read from the file the tooling generates —
          this page cannot disagree with the repository.
        </p>
        <AblationTable />
      </div>

      <p className="mt-7 max-w-2xl rounded-lg border border-white/[0.08] bg-white/[0.03] px-4 py-3 text-[12.5px] leading-relaxed text-neutral-400">
        <span className="font-semibold text-neutral-200">Ground truth:</span>{' '}
        drive where GNSS is good, record, then delete the fixes for a window
        before the estimator sees them. It cannot be fitted to an answer it was
        never shown.{' '}
        <span className="font-semibold text-neutral-200">
          One component ships switched off
        </span>{' '}
        — a forward-bias correction measured worse, 12.8% against 10.0%, and
        stays in the table with its real figure.
      </p>
    </Section>
  );
}
...
function Ai() {
  return (
    <Section id="ai" eyebrow="The AI" title="The one thing physics alone cannot answer.">
      ...
      <p className="mt-6 max-w-2xl text-[13px] leading-relaxed text-neutral-500">
        Mean error 3.7&nbsp;m/s, against 4.8 for ridge regression and 8.7 for
        assuming constant speed. All three reported — a model with no baseline
        is an unfalsifiable claim. Split{' '}
        <span className="text-neutral-300">by recording, never at random</span>.
        Inference is on-device: a cloud model needs a network, and where this
        app is needed there is none.
      </p>
...
function Get() {
  return (
    <section
      id="get"
      className="relative overflow-hidden border-t border-white/[0.06] bg-[#060910]"
    >
      <div className="pp-splash-glow pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2" />
      <div className="relative mx-auto max-w-3xl px-6 py-20 text-center">
        <p className="text-[10.5px] font-medium uppercase tracking-[0.2em] text-sky-400/80">
          Get it
        </p>
        <h2 className="mt-4 text-[2rem] font-bold leading-tight tracking-tight text-white sm:text-[2.4rem]">
          Put it on your phone.
        </h2>
        <p className="mx-auto mt-4 max-w-lg text-[14px] leading-relaxed text-neutral-400">
          Real IMU, real GNSS, offline maps, and the constraint switches you can
          break in front of anyone. The browser demo runs the same estimator —
          the app adds the sensors.
        </p>
        ...
        <div className="mt-9 flex flex-col items-center gap-5">
          ...
          <Link
            href="/"
            className="pp-press text-[13px] text-neutral-400 underline decoration-neutral-700 underline-offset-4 hover:text-neutral-200"
          >
            or open the browser demo
          </Link>
        </div>
      </div>
    </section>
