...
> ### Drift: **9.2% mean, 5.3% median, 22.6% p90** over 12 runs
>
...
### Field fix — "it goes off the road, into the plots" ✅

Reported from a real drive. The build it was reported against measured 10.0%
mean drift, inside target. Both statements were true at once, and that is the
whole lesson: drift is distance-to-truth and is blind to which SIDE of the
truth the error is on. 30 m along a road is invisible; 30 m across it is a
vehicle in somebody's field. Nothing measured the second thing, so nothing
caught it.

`pnpm eval:offroad` measures it now — ground truth is the road network, and
only samples the engine actually DREW while dead reckoning are counted:

|            | mean  | median | p90    | max     | >10m  | >25m  |
|------------|-------|--------|--------|---------|-------|-------|
| before     | 12.5m | 3.6m   | 46.6m  | 106.5m  | 26.9% | 12.5% |
| **after**  | 0.8m  | 0.0m   | 0.0m   | 71.1m   | 2.8%  | 0.8%  |
| snap off   | 15.7m | 5.6m   | 49.9m  | 106.5m  | 35.2% | 21.9% |

FOUR CAUSES, ALL IN ROAD SNAPPING, NONE VISIBLE IN THE DRIFT FIGURE

1. The snap was deliberately partial: `1 - confidence` clamped to [0.1, 0.7].
   On the first second of an outage confidence is still 1, so it applied a
   TENTH of the correction — at the exact moment the badge flips — and could
   never exceed 70%, so a permanent 30% of a growing error was always drawn.
   Under GNSS that gentleness is right (position is measured, the road is the
   weaker claim); the instant GNSS is gone it reverses. Full strength during
   DEAD_RECKONING only.
2. One fixed 50 m radius. Drift past it and the match returns null, so snapping
   switched itself off exactly when it was the only evidence left. A widened
   250 m search now runs when the ordinary one finds nothing.
3. Continuity survived running off the end of a way. Past a way's end the
   correction is almost entirely ALONG the road, which the snap discards by
   design — so it computed a 23 m error and moved the marker zero metres,
   leaving it in a field while the panel named the road it was not on.
4. "Cross-track only" had no escape hatch. At an endpoint, refusing to move
   ...
   end of a road. Now BOUNDED (25 m) rather than forbidden;
   `maxAlongCorrectionM: 0` restores the strict rule exactly.

Fixing it improved the headline too, because a marker held on the road is
closer to the truth: mean drift 10.3% -> 9.2%, median 7.6% -> 5.3%. Inside the
problem statement's <10% target.

ONE SELF-INFLICTED TELEPORT, FOUND BY MEASURING

Raising the snap to full strength introduced a 158.6 m single-sample jump when
the matched way changes — Golden Rule #6 broken by the fix for something else.
It was invisible because `maxUnexplainedJumpM` had never installed a road
graph, so the entire snap path was outside the reach of the teleport tests. The
correction is now a rate-limited vector capped at 60 m/s (the same bound the
recovery blender uses), the invariant tests run with a graph built to force a
way change mid-outage, and the worst unexplained step is 8 m — better than the
45.6 m the code had before any of this.

A second, subtler one: handing the recovery blender the already-snapped
position looked more correct ("slew from where the marker is") and double-
counted the offset, because step 8 applies the snap on top of whatever step 7
produces. 108.9 m. The blender works in unsnapped space, and now says so.
...
### Phase 13 Model 4, the skipped items, and Phase 16 finished ✅

Three things, in the order they were done.

★ MODEL 4 — THE GNSS QUALITY CLASSIFIER ★

The fourth and last model the PS asks for. Eleven features the receiver already
reports in; GOOD / MULTIPATH / SPOOFED / LOST out. 988 parameters, 6.1 KB.
Log-disjoint macro-F1 0.995 and 0.988 against a 0.250 chance baseline — AND
THAT NUMBER MEANS ALMOST NOTHING, because the labels are modelled corruptions
generated by a function in this same repository. The training script prints
that above the number rather than below it. Advisory only: it lowers the
confidence bar and can never gate a fix.

A unit test caught real train/serve skew. The first dataset made each label a
whole-log pass, so a LOST log had a baseline satellite count of one and
`satDropFromBaseline` was ZERO. The model learned that LOST means "terrible AND
always was". Fed an obviously dead receiver it answered SPOOFED. Every pass now
transitions out of GOOD at a randomised onset.

★ THE SKIPPED ITEMS, CLOSED ★

  - C/N0 mean AND spread from GnssStatus. Model 4's two best features had
    nothing populating them — the same inert-feature problem the barometer had.
    Computed over the satellites USED in the fix, not all in view: averaging
    the rejected ones reports the sky as permanently worse than the fix is.
  - Magnetometer, collected at 10 Hz and DELIBERATELY NOT USED. A car is a
    steel box; its own body distorts the field by tens of degrees and the
    distortion changes with heading, so a magnetic heading in a car is a
    heading plus an unknown function of where you are pointing. Written down
    rather than left as a silent omission.
  - Barometer wired (previous commit), which turned on the ESKF's altitude
    update and the HMM's flyover term — both of which had existed, been tested,
    and never once run.

★ PHASE 16 — FROM "A PROGRAM" TO "AN ENGINE" ★

It had a CLI, a simulator, a replay source, three grades and a benchmark. It
could not take an inertial stream off a wire or hand its estimate to anything,
which is the difference between a demo and something a manufacturer could
embed. Now:

  UdpImuSource      JSON datagrams. UDP and not TCP on purpose: a retransmitted
                    IMU sample arrives late, the engine treats an out-of-order
                    timestamp as a clock jump and discards it, so the
                    retransmission costs latency and buys nothing.
  SerialImuSource   UART, JSON or bare CSV. `serialport` is OPTIONAL — it
                    compiles a native addon, and requiring it would mean every
                    clone and every CI run builds C++ to run tests that never
                    open a port.
  output sinks      file, stdout, UDP broadcast, or any combination. Streamed
                    as they go: the first version accumulated every state in an
                    array and wrote at the end, so an interrupted run — which a
                    live stream frequently is — wrote nothing.
  Dockerfile        builds, and RUNS: 49,001 Hz sustained against a 200 Hz
                    target, 0.0896 ms p99 latency, inside the container.
  README            an "Edge deployment" section, with the one-core two-targets
                    diagram. A compulsory PS deliverable had NO documentation.

The Docker entrypoint took two attempts: `ENTRYPOINT ["/sbin/tini","--"]` alone
makes `docker run <image> --grade FOG` ask tini to execute a file called
"--grade", which it reports as a FATAL exec failure. The entrypoint has to name
the program.
