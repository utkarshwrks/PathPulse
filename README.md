...
> **📖 [MASTER.md](./MASTER.md) is the complete reference for this project** —
> every feature, the reasoning behind each one, the alternatives that were tried
> and measured, and why they lost. Start there.

## Status

**Build v0.22.** APK 7.41 MB · 1,588 tests · 60,224 lines. The dot survives a
GNSS outage under an uncertainty ellipse that shows how wrong it might be, the
map works offline anywhere in the world at ~3.5 MB per 100 km, and the estimator
is scored on real vehicle sensors as well as simulated ones.

| | Simulated (Tier S) | Real vehicle sensors (Tier R) |
| --- | --- | --- |
| Mean drift | **6.9 %** | **41.3 %** |
| Generated by | `pnpm ablation` → [`docs/benchmarks.md`](./docs/benchmarks.md) | `pnpm eval:tier-r` → [`docs/benchmarks-tier-r.md`](./docs/benchmarks-tier-r.md) |

Both are published because averaging them would be dishonest and hiding the
second would be worse. The ablation table also includes one component reported
as a negative result and shipped disabled. See
[MASTER.md §22, the honesty ledger](./MASTER.md#22--the-honesty-ledger).
