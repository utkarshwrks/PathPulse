> **Why there is no AI row in this table.** Both trained models are excluded
> deliberately, and the numbers that justify it are in
> [ml/README.md](../ml/README.md) — reproduce them with
> `python ml/check_sim_transfer.py`. On these SIMULATED logs the speed
> regressor scores 8–20 m/s MAE against 2.93 m/s on held-out real data, and
> the motion classifier scores 12.8 % accuracy against 57.4 %. Neither
> transfers to synthetic IMU, because the simulator's vibration is one 20 Hz
> sine plus Gaussian noise and both models key on a road and tyre spectrum
> that is not in there. A row from that would measure the simulator, not the
> model. **This is a statement about the logs, not about the models.**
